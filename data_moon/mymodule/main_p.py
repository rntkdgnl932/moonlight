...
from function_moon import imgs_set, imgs_set_, click_pos_2, random_int, text_check_get_3, int_put_, text_check_get, click_with_image, drag_pos, image_processing, get_region, click_pos_reg, win_left_move, win_right_move
...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\moonlight\\data_moon;./data_moon" --add-data="C:\\my_games\\moonlight\\mysettings;./mysettings" --name moonlight -i="moonlight_macro.ico" --add-data="moonlight_macro.ico;./" --icon="moonlight_macro.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
        ...
        self.show()
    def my_title(self):
        self.setWindowTitle("달빛조각사(ver " + version + ")")
...
class ThirdTab(QWidget):
    ...
    def button_line_save(self):
        ...
        dir_path = "C:\\my_games"
        file_path = dir_path + "\\line\\line.txt"
        file_path2 = dir_path + "\\moonlight\\mysettings\\game_server\\game_server.txt"
...
class SecondTab(QWidget):
    ...
    def win_left_ex(self):
        print("왼쪽으로 정렬 합니다.")
        full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\moonlight_title.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(960, 0, 1920, 1080, "three", img, 0.8)
        time.sleep(0.3)
        if imgs_ is not None and imgs_ != False:
            print("왼쪽 moonlight 보여", imgs_)
            time.sleep(0.5)
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        file_path5 = dir_path + "\\data_moon\\jadong\\jadong_force_list.txt"
        ...
        dir_path = "C:\\my_games\\moonlight\\mysettings\\my_level"
        ...
        dun_g1_list = ['균열의 땅 선택', '홍염의신전', '얼음유적지', '마리아스의동굴', '연회장']
    ...
    def moonlight_stop_perfect(self):
        try:
            print("game_Playing(self): moonlight_stop")
            dir_path = "C:\\my_games\\load\\moonlight"
    ...
    def onActivated_slelect_spot(self, e):
        if e != 0 and e != '사냥터':
            ...
            dir_path = "C:\\my_games\\moonlight"
            dir_spot = "C:\\my_games\\moonlight\\mysettings\\gold_force"
    ...
    def onActivated_slelect_gold_read(self):
        dir_path = "C:\\my_games\\moonlight"
        dir_gold = "C:\\my_games\\moonlight\\mysettings\\gold_force"
    ...
    def onActivated_slelect_collection_toggle_read(self):
        ...
        dir_path = "C:\\my_games\\moonlight"
        dir_toggle = "C:\\my_games\\moonlight\\mysettings\\collection"
...
class game_Playing(QThread):
    ...
    def run(self):
        ...
        try:
            print("moonlight go", v_.now_cla)

            while self.isCheck is True:


                print("달빛조각사 실행 모드(ver " + version + ")")
                print("moonlight cla", v_.now_cla)
                print("now_arduino", v_.now_arduino)
                result_game = game_start()
                if result_game == True and v_.now_cla != "none":
                    # 이전 게임 모드 불러와서 실행
                    full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\touching.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\game_moonlight_title.PNG"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        ...
                        print()
                        full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\game_moonlight_title_2.PNG"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set_(0, 0, 960, 1030, v_.now_cla, img, 0.8)
                        if imgs_ is not None and imgs_ != False:
                        ...
                        if game_title == True:
                            ...
                            full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\moonlight_start_ready.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 50, 960, 1030, v_.now_cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                look_title = True
                            full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\moonlight_start_ready_2.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 50, 960, 1030, v_.now_cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                            ...
                            if look_title == True:
                                ...
                                for i in range(10):
                                    ...
                                    if imgs_ is not None and imgs_ != False:
                                        if i > 8:
                                            full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\moonlight_title_2.PNG"
                                            img_array = np.fromfile(full_path, np.uint8)
                                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                            # 아레스는 1클라 고정
                                            imgs_ = imgs_set_(0, 50, 960, 1030, v_.now_cla, img, 0.7)
                            ...
                            print("달빛조각사 꺼져있는지 10초간 다시 검사하기")
                            is_moonlight = False
