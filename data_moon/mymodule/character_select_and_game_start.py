...
def game_start_screen(cla, character_id):
    ...
    try:
        ...
        # 실수로 새 캐릭터 클릭시...
        full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\game_start\\job_select_title.PNG"
...
def character_change(cla, character_id):
    ...
    try:
        ...
        while cha_select is False:
            ...
            imgs_ = imgs_set_(20, 30, 150, 100, cla, img, 0.8)
            if imgs_ is not None and imgs_ != False:
                full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\game_start\\game_start_button.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def game_ready(cla):
    ...
    try:
        ...
        # 접속대기일 경우 기다리기
        full_path = "c:\\my_games\\moonlight\\data_moon\\imgs\\check\\moon_game_ready.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
