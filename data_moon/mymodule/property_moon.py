...
def my_property_upload(cla):
    ...
    import os
    import numpy as np
    from action_moon import mine_check, out_check, clean_screen
    ...
    try:
        ...
        if int(result_mine[1]) != 0:
            ...
            # 6. 로컬 파일 경로 (절대 경로 사용)
            local_file_path = 'C:/my_games/moonlight/mysettings/my_property/my_property.txt'

            dir_path = "C:\\my_games\\moonlight\\mysettings\\my_property"
            ...
            # 7. 원격 파일 경로 (FTP 서버 내)
            remote_directory = '/moonlight/' + str(line_[0])  # 원격 디렉토리 경로
            remote_file_path = '/moonlight/' + str(line_[0]) + '/' + str(line_[1]) + '.txt'
